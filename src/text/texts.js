const texts = [

    `I’m a full stack JavaScript developer living in Stockholm with a great interest in tech and music. I have been running an event business for several years and now changed careers to follow my passion for web development.

    In addition to my previous employments, I have for several years run the event company NOPH Events together with a colleague. In this industry, it is very much about finding solutions, being creative and flexible, which has suited me as I like to see opportunities in challenges.

    As a person, I am active and spend a lot of time with family and friends where football, golf and paddle are some of the activities we like to spend together.`,

    `Full Stack JavaScript - School of Applied
    Technology </salt>
    May 2021 - Jul 2021
    Intensive three month training program for full stack web development with a focus on TDD, mob programming, and applied learning.
    
    Python – Blekinge Tekniska Högskola
    Aug 2019 - Okt 2019
    Programming and problem solving with Python.

    JavaScript – Blekinge Tekniska Högskola
    Okt 2019 - Nov 2019
    Programming with JavaScript
    `,

    `Substitute teacher – Sticklinge Skola
    Sep 2020 - Apr 2021 
    Substitute teacher, grade F-6. Weekly lessons in  basic programming in third grade.
    
    Event manager/founder – NOPH Events
    11 2015 - 
    Planning and organizing events such as weddings, parties and commercial launchings.
    
    Coach/advisor – Zensum
    Aug 2015 - Jun 2016 
    Supervisor, I handled applications, was a coach for evening and weekend workers and advised customers.
    `
]

export default texts;